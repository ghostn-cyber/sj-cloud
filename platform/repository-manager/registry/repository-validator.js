...
class RepositoryValidator {
  constructor() {
    ...
    const schemaPath = path.join(__dirname, 'repository-schema.yaml');
  ...
  validate(config) {
    ...
    if (!result.valid) {
      ...
      throw new ValidationError(`Repository validation failed: ${errMsgs}`, { details: { errors: result.errors } });
    }
    return true;
  }
}
