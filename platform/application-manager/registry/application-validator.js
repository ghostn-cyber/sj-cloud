...
class ApplicationValidator {
  constructor() {
    ...
    const schemaPath = path.join(__dirname, 'application-schema.yaml');
  ...
  validate(config) {
    ...
    if (!result.valid) {
      ...
      throw new ValidationError(`Application validation failed: ${errMsgs}`, { details: { errors: result.errors } });
    }
    return true;
  }
}
