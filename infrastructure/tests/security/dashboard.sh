...
STATUS_UNAUTH=$(curl -k -s -o /dev/null -w "%{http_code}" --resolve "traefik.sj-cloud.test:443:127.0.0.1" https://traefik.sj-cloud.test/dashboard/)
...
STATUS_AUTH=$(curl -k -s -o /dev/null -w "%{http_code}" -u "admin:sjcloudadmin" --resolve "traefik.sj-cloud.test:443:127.0.0.1" https://traefik.sj-cloud.test/dashboard/)
...
HEADERS=$(curl -k -s -I -u "admin:sjcloudadmin" --resolve "traefik.sj-cloud.test:443:127.0.0.1" https://traefik.sj-cloud.test/dashboard/ | tr '[:upper:]' '[:lower:]')
